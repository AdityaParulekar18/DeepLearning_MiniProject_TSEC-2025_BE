...
APPROACHING_THRESHOLD = 0.04     
DISTANCE_BUFFER_SIZE = 4         
ALERT_COOLDOWN = 7 
...
def speak(text):
    """RE-IMPLEMENTED: Initializes and runs the Text-to-Speech engine."""
    print(f"🔊 [SPOKEN]: {text}")
    try:
        engine = pyttsx3.init()
        ...
        # This block catches errors if pyttsx3 can't connect (common in SSH/remote environments)
...
def main_inference():
    ...
    #cap = cv2.VideoCapture('https://10.175.192.119:8080/video') 
 
    if not cap.isOpened():
        print("Error: Could not open video stream. Check camera ID or IP address.")
        return
        ...
        if approaching_person_count > 0 and closest_distance < float('inf'):
            ...
            if current_time - last_alert_time > ALERT_COOLDOWN:
                ...
                if is_initial or distance_changed or count_changed:
                    ...
                    speak(alert_text) # <-- SPEAK FUNCTION CALLED
            ...
            if current_time - last_alert_time > 15 and LAST_SPOKEN_STATUS != "clear":
                 speak("Path is clear.") # <-- SPEAK FUNCTION CALLED
    ...
    speak("System shutting down.") # <-- SPEAK FUNCTION CALLED
...
if __name__ == "__main__":
    if not os.path.exists("basic_detector_model.pth"):
        ...
        # Note: You need to install pyttsx3 before running: pip install pyttsx3
        main_inference()